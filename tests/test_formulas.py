...
class TestKelly:
    """kelly_stake() with quarter-Kelly and 5% max-risk cap."""

    def _cfg(self, kf=0.25, max_risk=0.05, min_stake=1.0):
        from core.bankroll import KellyConfig
        return KellyConfig(kelly_fraction=kf, max_risk_pct=max_risk, min_stake=min_stake)

    def test_known_stake(self):
        from core.bankroll import kelly_stake
        # odds=2.00, prob=0.55, bankroll=1000, kf=0.25, max_risk=0.05
        # b=1.0, edge=(1.0*0.55 - 0.45)/1.0 = 0.10
        # stake_raw = 1000 * 0.10 * 0.25 = 25.0
        # stake_cap = 1000 * 0.05 = 50.0 → result = 25.0
        result = kelly_stake(2.00, 0.55, 1000.0, self._cfg())
        assert result == pytest.approx(25.0)

    def test_max_risk_cap_binds(self):
        from core.bankroll import kelly_stake
        # Huge edge: prob=0.90, odds=5.00 → huge raw Kelly, capped at 5%
        result = kelly_stake(5.00, 0.90, 1000.0, self._cfg())
        assert result <= 1000.0 * 0.05 + 1e-6  # never exceeds cap

    def test_negative_ev_returns_zero(self):
        from core.bankroll import kelly_stake
        # prob=0.40, odds=2.00 → edge = (0.40 - 0.60)/1.0 = -0.20 → no bet
        result = kelly_stake(2.00, 0.40, 1000.0, self._cfg())
        assert result == 0.0

    def test_zero_bankroll_returns_zero(self):
        from core.bankroll import kelly_stake
        assert kelly_stake(2.00, 0.55, 0.0, self._cfg()) == 0.0

    def test_odds_at_or_below_1_returns_zero(self):
        from core.bankroll import kelly_stake
        assert kelly_stake(1.0, 0.55, 1000.0, self._cfg()) == 0.0
        assert kelly_stake(0.9, 0.55, 1000.0, self._cfg()) == 0.0

    def test_full_kelly_is_4x_quarter_kelly(self):
        from core.bankroll import kelly_stake
        full    = kelly_stake(2.00, 0.55, 1000.0, self._cfg(kf=1.0, max_risk=1.0))
        quarter = kelly_stake(2.00, 0.55, 1000.0, self._cfg(kf=0.25, max_risk=1.0))
        assert abs(full - 4 * quarter) < 1e-6

    def test_min_stake_applied_when_edge_positive(self):
        from core.bankroll import kelly_stake
        # Very small bankroll: raw Kelly tiny, but edge exists → min_stake applies
        result = kelly_stake(2.00, 0.55, 1.0, self._cfg(min_stake=1.0, max_risk=1.0))
        assert result >= 1.0

    def test_proportional_to_bankroll(self):
        from core.bankroll import kelly_stake
        cfg = self._cfg(max_risk=1.0)  # remove cap so it's pure Kelly
        r1 = kelly_stake(2.00, 0.55, 1000.0, cfg)
        r2 = kelly_stake(2.00, 0.55, 2000.0, cfg)
