"""
FanGraphs pitcher + batter leaderboard fetcher.

Uses the free public JSON API (no auth required). Returns real xFIP, SIERA,
WAR, FIP, K%, BB%, SwStr% etc. for pitchers, and BB%, K%, wRC+ etc. for
batters — all keyed by MLBAM player_id (xMLBAMID field).

Name and Team columns contain HTML anchors — stripped before returning.

Endpoint (same URL, `stats=pit` or `stats=bat`):
...
class FanGraphsFetcher:
    """Fetches and caches FanGraphs pitcher and batter leaderboard data."""
    ...
    def get_batter_stats(self, mlbam_id: int, year: int) -> Dict:
        """Return FanGraphs stats dict for one batter (empty dict if not found)."""
        all_stats = self.get_all_batter_stats(year)
        return all_stats.get(int(mlbam_id), {})

    def get_all_batter_stats(self, year: int) -> Dict[int, Dict]:
        """Return dict keyed by MLBAM player_id for all batters in leaderboard."""
        cache_key = f"fg_batters_{year}"
        cached = self._load_cache(cache_key)
        if cached is not None:
            return cached

        logger.info(f"[fangraphs] downloading batter leaderboard ({year})...")
        try:
            r = self._session.get(
                _FG_URL,
                params={
            ...
            data = r.json()
            rows = data.get("data", [])
            result = self._parse_batter_rows(rows)
            self._save_cache(cache_key, result)
            logger.info(f"[fangraphs] leaderboard: {len(result)} batters")
            return result
        except Exception as exc:
            logger.warning(f"[fangraphs] batter fetch failed: {exc}")
            return {}
    ...
    def _parse_batter_rows(self, rows) -> Dict[int, Dict]:
        out: Dict[int, Dict] = {}
        for row in rows:
            ...
            out[mlbam_id] = {
                # Plate discipline — player-level, for lineup-filtered TTE blending
                "pa":           _f(row.get("PA")),
                "bb_pct":       _f(row.get("BB%")),
                "k_pct":        _f(row.get("K%")),
                "wrc_plus":     _f(row.get("wRC+")),
                # Cross-reference fields (Savant already covers these; kept for parity)
